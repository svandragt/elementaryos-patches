...
// core.<comm>.<uid>.<bootid>.<pid>.<usec>[.zst|.lz4|.xz], then regenerates
// the signal and backtrace straight from the core file with gdb: the core's
// notes still name the executable and terminating signal even without the
// journal entry. Only the journal context (surrounding log lines) is truly
// unrecoverable.
// ---------------------------------------------------------------------------
...
func pollCoredumpDir(ctx context.Context, store *Store, known map[string]bool) {
	...
	for _, entry := range entries {
		...
		exe, sig, pkg := comm, "(unknown signal)", ""
		backtrace := "(recovered from " + coredumpDir + "; journal entry expired, no backtrace available)"
		rexe, rsig, bt, err := recoverFromCoreFile(ctx, filepath.Join(coredumpDir, entry.Name()))
		if rexe != "" {
			exe = rexe
			pkg = packageOf(ctx, exe)
		}
		...
		if err != nil {
			log.Printf("coredumpdir: recover %s: %v", entry.Name(), err)
		} else {
			backtrace = "(recovered from " + coredumpDir + "; journal entry expired, backtrace regenerated with gdb)\n\n" + bt
		}
		// the coredump journal entry is gone, but ordinary log lines from
		// around the crash may have outlived it — worth attempting
		journal := journalContext(ctx, pid, occurredAt)
		created, err := store.recordCrash(exe, sig, topFrameOf(backtrace),
			sourceKey, "coredumpdir", occurredAt, pid, uid, backtrace, journal, pkg)
		if err != nil {
			log.Printf("coredumpdir: record crash: %v", err)
			continue
		}
		if created {
			log.Printf("coredumpdir: recovered crash exe=%s signal=%s pid=%d at=%s", exe, sig, pid, occurredAt.Format(time.RFC3339))
		}
	}
}

// coreProbeExe matches gdb's "Core was generated by `/usr/bin/gala --arg'."
// line; coreProbeSig matches "Program terminated with signal SIGSEGV, ...".
var (
	coreProbeExe = regexp.MustCompile("Core was generated by `([^ '‘’]+)")
	coreProbeSig = regexp.MustCompile(`terminated with signal (SIG[A-Z0-9]+|[0-9]+)`)
)

// recoverFromCoreFile regenerates executable path, signal, and backtrace from
// a raw core file whose journal metadata has expired. A first gdb pass over
// the core alone reads both from the core's notes; a second pass with the
// executable produces the full backtrace (same script as the coredumpctl
// path, symbols via debuginfod). On error, exe and sig are still returned if
// the probe pass recovered them — a core whose binary is gone (upgraded
// package, flatpak sandbox path) can still yield the signal.
func recoverFromCoreFile(ctx context.Context, path string) (exe, sig, backtrace string, err error) {
	...
	core, cleanup, err := decompressCore(ctx, path)
	if err != nil {
		return "", "", "", err
	}
	defer cleanup()

	probe, _ := exec.CommandContext(ctx, "gdb", "-batch", "-nx", "-c", core).CombinedOutput()
	sig = ""
	if m := coreProbeSig.FindSubmatch(probe); m != nil {
		sig = normalizeSignal(string(m[1]))
	}
	m := coreProbeExe.FindSubmatch(probe)
	if m == nil {
		return "", sig, "", fmt.Errorf("could not determine executable: %s", firstLine(string(probe)))
	}
	exe = string(m[1])
	if !filepath.IsAbs(exe) {
		// argv[0] was a bare command name; resolve it the way the shell did
		if resolved, err := exec.LookPath(exe); err == nil {
			exe = resolved
		}
	}
	if _, err := os.Stat(exe); err != nil {
		return exe, sig, "", fmt.Errorf("executable no longer present: %v", err)
	}
	...
	cmd := exec.CommandContext(ctx, "gdb", "-batch", "-nx", "-x", script.Name(), exe, core)
	if os.Getenv("DEBUGINFOD_URLS") == "" {
		cmd.Env = append(os.Environ(), "DEBUGINFOD_URLS=https://debuginfod.ubuntu.com")
	}
	out, err := cmd.CombinedOutput()
	...
	return exe, sig, string(out), nil
}

// decompressCore returns a path gdb can read directly, decompressing
// .zst/.lz4/.xz cores to a temp file. cleanup removes the temp file (a no-op
// for already-uncompressed cores).
func decompressCore(ctx context.Context, path string) (core string, cleanup func(), err error) {
	var tool string
	...
	tmp, err := os.CreateTemp("", "crash-dashboard-core-*")
	if err != nil {
		return "", nil, err
	}
	cmd := exec.CommandContext(ctx, tool, "-dqc", path)
	cmd.Stdout = tmp
	runErr := cmd.Run()
	closeErr := tmp.Close()
	if runErr != nil || closeErr != nil {
		...
		return "", nil, fmt.Errorf("decompress %s: %v", filepath.Base(path), runErr)
	}
...
func main() {
	...
	go func() {
		...
		for {
			...
			pollCoredumpDir(ctx, store, known)
