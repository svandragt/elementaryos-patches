...
|---------|---------|-------------|
| [io.elementary.notifications](pkgs/io.elementary.notifications/) | [series](pkgs/io.elementary.notifications/series) | <ul><li>Fix bubbles that never appear, or appear as a rectangle of stale screen content. The carousel's initial `scroll_to` is a no-op until the widget has an allocation, which a toplevel lacks at map time, so the bubble can stay on the blank placeholder page — leaving the window unpainted, and `page_changed` reading it as a swipe-to-dismiss. Re-assert `scroll_to` from a tick callback until it lands, and only trust a page mismatch as a dismissal once the overlay page has actually been reached.</li></ul><br>Focus stealing is handled compositor-side by gala's `fix-notification-focus-steal-x11.patch`, not here. |
