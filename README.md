...
|---------|---------|-------------|
| [io.elementary.notifications](pkgs/io.elementary.notifications/) | [series](pkgs/io.elementary.notifications/series) | Don't steal focus when a notification bubble appears on X11; fix notifications silently dropped after sleep/resume (GDK frame clock not ready, retry until compositor is up); fix `realize()`/`unrealize()` resolving to the wrong `Gtk.Native` interface method instead of `Gtk.Widget` (made the retry loop above fail 100% of the time instead of only after sleep/resume); fix bubbles silently dismissed when the per-app "sounds" setting is off (carousel page-changed race unmasked by skipping the sound call) |
