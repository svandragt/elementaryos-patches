...
|---------|---------|-------------|
| [io.elementary.notifications](pkgs/io.elementary.notifications/) | [series](pkgs/io.elementary.notifications/series) | Don't steal focus when a notification bubble appears on X11 (set `_NET_WM_WINDOW_TYPE` and user time 0 on the window's own surface before mapping — best-effort, GDK's map-time writes race the WM; the airtight fix is gala's `fix-notification-focus-steal-x11.patch`); fix notifications silently dropped after sleep/resume (GDK frame clock not ready, retry until compositor is up); fix `realize()`/`unrealize()` resolving to the wrong `Gtk.Native` interface method instead of `Gtk.Widget` (made the retry loop above fail 100% of the time instead of only after sleep/resume); fix bubbles silently dismissed when the per-app "sounds" setting is off (carousel page-changed race unmasked by skipping the sound call) |
| [gala](pkgs/gala/) | [series](pkgs/gala/series) | Stop crash loop from unbalanced `WorkspaceManager.thaw_remove` when a workspace is added outside a swipe gesture; prevent focus-stealing on attention demand; never let notification bubbles hold keyboard focus on X11 (mutter's focus-on-map races the daemon's user-time hint — bounce focus back to the MRU window) |
