...
|---------|---------|-------------|
| [io.elementary.notifications](pkgs/io.elementary.notifications/) | [series](pkgs/io.elementary.notifications/series) | <ul><li>Fix bubbles that never appear, or appear as a rectangle of stale screen content. The carousel's initial `scroll_to` is a no-op until the widget has an allocation, which a toplevel lacks at map time, so the bubble can stay on the blank placeholder page — leaving the window unpainted, and `page_changed` reading it as a swipe-to-dismiss. Re-assert `scroll_to` from a tick callback until it lands, and only trust a page mismatch as a dismissal once the overlay page has actually been reached.</li><li>Keep the 5 most recent bubbles on screen instead of auto-hiding after 4s; only evict the oldest once a 6th arrives. Urgent notifications are never evicted.</li></ul><br>Focus stealing is handled compositor-side by gala's `fix-notification-focus-steal-x11.patch`, not here. |
| [gala](pkgs/gala/) | [series](pkgs/gala/series) | <ul><li>Stop crash loop from unbalanced `WorkspaceManager.thaw_remove` when a workspace is added outside a swipe gesture</li><li>Prevent focus-stealing on attention demand</li><li>Never let notification bubbles hold keyboard focus on X11 (mutter's focus-on-map races the daemon's user-time hint — bounce focus back to the MRU window)</li><li>Stop the crash when dismissing a notification bubble. Upstream 8.6.1's use-after-free guard only fires for `NOTIFICATION`-type windows, but `NotificationsClient` rewrites bubbles to `DOCK`, so `destroy_completed` still ran on a freed actor</li><li>Fix the title-bar right-click menu opening as an empty bubble. Gala exported the menu object from its name-acquired callback, so the name appeared on the bus a moment before the object did; a client that built its `GDBusMenuModel` in that gap had its subscription refused and GLib never retried, leaving the menu empty for the life of the daemon. Export the model and action group before owning the name, and watch the name daemon-side so a gala restart rebuilds the menu</li><li>Stop the crash loop on gala startup: `x11_set_window_pass_through` dereferenced a null shape-rectangle list for a shell window whose X window was already gone, and `thaw_remove` aborted on an unmatched thaw from the workspace-added jump path</li></ul> |
